...
export default {
    ...
    /**
     * Checks if used browser is a mobile.
     * Inspired by http://stackoverflow.com/a/11381730/989439
     *
     * @return {boolean}
     */
    isMobile() {
...
function getBrowserData() {
    if (!browserData) {
        ...
        let match = userAgent.match(/(opera|opr|chrome|safari|firefox|msie)\/?\s*(\.?\d+(\.\d+)*)/i),
            versionMatch;

        if (match && (versionMatch = userAgent.match(/version\/([\.\d]+)/i)) !== null) {
            match[2] = versionMatch[1];
        }

        match = match ? [match[1], match[2]] : [navigator.appName, navigator.appVersion];
        browserData = {
            name: match[0].toLowerCase().replace('opr', 'opera'),
            version: match[1]
        }
    }
